...
function searchCards (repositories) { // создание карточек автокомплита
    ...
    for(el of repositories) {
        ...
        // console.log({ owner, name, stargazers_count } )
    ...
    ul.append(fragment)
    app.append(ul)
    // tempRepos(repositories)
}

// const temporaryRepositories = function tempRepos(repositories) { // временное хранилище репозиториев
//     return repositories
// }
...
//     li.removeEventListener('click', addResult)
//     return resultRepos
// }
...
function resultRepos(res) {
    ...
    li.textContent = `Name: ${res[0].name} 
    Owner: ${res[0].owner.type}
    Stars: ${res[0].stargazers_count}`;
    fragment.append(li);
    ul.append(fragment)
    resCard.append(ul)
}
...
document.addEventListener('click', (e) => { 
    ...
    //     // let repos = tempRepos();
    //     // console.log(repos)
    //     let result = repos.filter((e) => e.name == link)
